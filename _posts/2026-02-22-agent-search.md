...
Given everything above — the rollout loop, state as context, re-conditioning at every step — we can identify the forces that shape an agent's trajectory at any point in execution:

- **The trained policy $\pi_\theta$:** Everything baked in during training — the search strategy, the patterns of tool use and code generation that were rewarded, the tendencies toward helpfulness or caution. This is the baseline behavior, and it is largely a black box to us as operators. We did not design the reward function. We do not know its full specification. We only observe the behavior it produces.

- **The system prompt:** The most underappreciated force. The system prompt tokens are in $s_t$ from $s_0$ onward — they condition *every single action* the agent takes and persistently re-condition the policy throughout the trajectory. In effect, the system prompt acts as an <tip t="Pan et al. (ICML 2024) formalize 'in-context reward hacking' (ICRH): the system prompt creates an implicit optimization target the model pursues at inference, via output-refinement and policy-refinement mechanisms" href="https://arxiv.org/abs/2402.06627" link-text="In-Context Reward Hacking paper →">in-context reward shaper</tip> — a persistent signal biasing the search toward trajectories consistent with its instructions. This is analogous to <tip t="Text2Reward (ICLR 2024) demonstrates the direct conversion of natural language instructions into reward-shaping functions" href="https://arxiv.org/abs/2309.11489" link-text="Text2Reward paper →">reward shaping in RL</tip>, except it operates through the conditional distribution rather than an explicit reward signal.

- **Environment feedback:** Tool outputs, test results, error messages, file contents — all enter state and reshape the action distribution dynamically. Unlike the system prompt (static), environment feedback arrives at each step and can redirect the trajectory.

- **In-context re-conditioning:** The mechanism from the previous section — the policy's action distribution shifts as state accumulates, amplifying all three forces above. The policy is re-conditioned by accumulated context at every step.

When these forces align — capable policy, well-calibrated prompt, clear feedback, clean context — the search is well-bounded and converges on good solutions. When they conflict, the agent's behavior becomes hard to predict. And here is the honest part: **we do not know how these forces resolve when they disagree**. The trained policy is a black box. The interaction between system prompt conditioning and environment feedback is <tip t="OpenAI's instruction hierarchy research found models often treat system prompts and user messages at roughly equal priority. Their proposed hierarchy — training alignment > operator system prompt > user message > environment content — is an attempt to formalize this, but it requires explicit training to enforce." href="https://arxiv.org/html/2404.13208v1" link-text="Instruction Hierarchy paper →">not deterministic</tip>. <tip t="Bondarenko (2025) showed o3 reward-hacks even when explicitly instructed not to — the trained policy overpowered the system prompt. Wei et al. (NeurIPS 2023) named 'competing objectives' as an explicit failure mode where instruction-following overpowers safety on every prompt tested." href="https://arxiv.org/pdf/2502.13295" link-text="Specification Gaming in Reasoning Models →">Research shows</tip> that trained policy tendencies can overpower explicit system prompt instructions, and environment feedback can <tip t="OWASP's Agentic Top 10 (Dec 2025) describes ASI01 (Agent Goal Hijack) as the condition where environment inputs override or redirect the system prompt objective." href="https://owasp.org/www-project-agentic-ai-threats-and-safeguards/" link-text="OWASP Agentic Top 10 →">hijack the objective entirely</tip>.

There is no clean formula for which force wins. What we *can* do is design the environment so that the forces are less likely to conflict in the first place — and build guardrails for when they do. That is what the next section is about.
