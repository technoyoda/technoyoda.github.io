...
The key insight is the same one Lewin had: the agent responds to the **total state**, not to any single input. The system prompt is one force, the environment's most recent feedback is another, the trained policy's biases are a third. They all act on $s_t$ simultaneously. You cannot decompose "this much came from the prompt, this much from the environment." You can only observe the total effect. And because $s_t$ changes at every step, the life space — the search landscape — shifts continuously as the agent acts and receives feedback.
