...
### <tip t="The name is inspired by Kurt Lewin's Field Theory (1936) — the idea that behavior is a function of the person and their environment, B = f(P, E), and can only be understood by examining the forces acting within the total situation." href="https://en.wikipedia.org/wiki/Field_theory_(psychology)" link-text="Lewin's Field Theory →">Agent Field Theory</tip>

Given everything above — the rollout, state as context, re-conditioning — we can now describe what actually determines agent behavior. There are three components, but they are not independent forces pushing on a ball. They form a causal chain with feedback loops.
...
<!-- TODO: Excalidraw diagram of the interaction loop: system prompt + environment → state → policy → action → environment feedback → state update → re-conditioning → loop -->
