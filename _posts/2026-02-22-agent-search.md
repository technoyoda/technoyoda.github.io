...
Critically, models do not just passively sample from context — they adapt their behavior based on patterns in $s_t$, without any weight updates. As the context accumulates information, the model's action distribution shifts in response.
<!-- TODO: [rework ICL citation] The current framing blurs two claims: (1) models adapt to patterns in context (well-established), and (2) models do in-context *reinforcement learning* from implicit rewards (stronger, less established). The cited paper (2410.05362) studies explicit reward signals in contextual bandit settings — not the implicit adaptation from tool outputs and error messages that coding agents do. There is an Anthropic research study on this that we should find and cite instead. For now, the argument doesn't fall apart without the strong ICL-as-RL claim — the weaker "models adapt to context patterns" is sufficient for our thesis. -->
<tip t="[TODO: find and cite Anthropic's research on in-context learning. Current citation (2410.05362) studies explicit reward signals in bandit settings — our claim about implicit adaptation from tool outputs is related but distinct.]">[TODO: better ICL citation needed]</tip> As the agent reads files, observes tool outputs, and encounters errors, it refines its search strategy in real time. The agent is not just conditioned by its context — it is *learning from it* on every step.
