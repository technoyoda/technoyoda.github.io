...
There is one more dynamic to account for. Models do not merely execute a fixed policy conditioned on context. They exhibit <tip t="'LLMs Are In-Context Reinforcement Learners' (2024) investigates contextual bandit settings where models learn in-context from external reward signals rather than supervised demonstrations" href="https://arxiv.org/abs/2410.05362" link-text="In-Context RL paper →">in-context learning</tip>: the ability to adapt their behavior based on patterns in the context itself, without any weight updates.
