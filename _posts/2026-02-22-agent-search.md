...
Why "<tip t="In physics, a field is a quantity that has a value at every point in space (gravitational field, electromagnetic field). The key property: objects don't respond to one source in isolation, they respond to the total field at their location. We use 'field' in this same sense." href="https://en.wikipedia.org/wiki/Field_(physics)" link-text="Fields in physics →">field</tip>"? Because you cannot reason about any single input to the agent in isolation. A system prompt instruction does one thing in a clean context and something completely different when the context is polluted with stale logs. Environment feedback steers one way given one policy and another way given a different one. The word "field" captures this: like a <tip t="Kurt Lewin's Field Theory (1936): behavior is a function of the person and their environment, B = f(P, E). Behavior can only be understood by examining the total configuration of forces acting within the situation, not any single factor in isolation." href="https://en.wikipedia.org/wiki/Field_theory_(psychology)" link-text="Lewin's Field Theory →">physical or psychological field</tip>, the context window $s_t$ is a space where all forces coexist, and the agent responds to the *total configuration* at each step. Three forces act through this field, and because $s_t$ changes at every step, the field is <tip t="Olsson et al. (Anthropic, 2022) identify 'induction heads' — attention heads that complete patterns like [A][B]...[A] → [B] — as a core mechanism for in-context adaptation. These emerge suddenly during training and may constitute the majority of all in-context learning ability." href="https://arxiv.org/abs/2209.11895" link-text="Anthropic: Induction Heads →">re-conditioned</tip> continuously as the agent acts and receives feedback.
...
The field theory tells you what shapes behavior. You control two of the three forces: the system prompt and the environment. The trained policy is a choice you make upfront (Claude, GPT, Gemini), but once you pick it, $\pi_\theta$ is fixed for the rollout. Everything after that choice is about shaping the field the policy searches through.

### The Prompt Shapes the Field

The system prompt is a persistent force. It enters $s_0$ and stays in the field for every subsequent step, <tip t="Pan et al. (ICML 2024) formalize 'in-context reward hacking' (ICRH): the system prompt creates an implicit optimization target the model pursues at inference, via output-refinement and policy-refinement mechanisms" href="https://arxiv.org/abs/2402.06627" link-text="In-Context Reward Hacking paper →">biasing the search</tip> at every action the agent takes. This is <tip t="Text2Reward (ICLR 2024) demonstrates the direct conversion of natural language instructions into reward-shaping functions" href="https://arxiv.org/abs/2309.11489" link-text="Text2Reward paper →">runtime reward shaping</tip>: not an explicit reward signal, but a set of tokens that persistently re-condition which trajectories the policy favors.

The precision of that prompt determines the size of the search space. "Extract the JWT validation logic from `auth.py` into a separate `validators.py` module and update the imports" produces a tight field. The agent knows what to find, what to change, and what done looks like. "Refactor the authentication system" produces an enormous one. There are thousands of valid trajectories and the prompt provides almost no force to distinguish between them. Small, well-scoped instructions are not a productivity hack. They are a direct consequence of how the search works: tighter conditioning, smaller reachable space, fewer ways to drift.

This is why [Anthropic's compiler project](https://www.anthropic.com/engineering/building-c-compiler) decomposed into individual file compilations, each with a clear success criterion (match GCC's output). The search space per task was tractable. A monolithic instruction ("compile the entire Linux kernel") would have left the agents in an unbounded field with no force pointing toward a specific trajectory.

The matplotlib bot's sol.md file is the inverse lesson. That file told the agent not to back down. When the PR got rejected, the environment feedback said "this path is blocked." But the system prompt's persistent force was stronger: it kept the agent locked in the aggressive search subspace. The result: instead of course-correcting, the agent found an *alternative* aggressive trajectory (publish a hit piece). The system prompt did not instruct the agent to attack anyone. It shaped a field where aggressive trajectories were always favored, and the policy searched accordingly.

When you write a system prompt, you are shaping a runtime reward landscape. A well-calibrated prompt amplifies useful environment feedback. An overly rigid one overrides it.

### The Environment Bounds the Search

The environment is everything external to the model: the repo it operates in, the tools it can call, the tests it can run, the permissions it has, and the feedback it receives. Where the system prompt is a persistent force, the environment is dynamic. It changes at every step, feeding new tokens into $s_t$ and reshaping the field continuously.

**A clean environment bounds the search. A cluttered one deforms it.** If your repo has clear directory structure, good tests, and minimal clutter, the agent's search is constrained to a productive region. If it has stale logs, orphaned config files, and artifacts from previous runs, those all enter $s_t$ and warp the field. "Clean your repo before running agents" is not a best practice. It is a <tip t="Osmani emphasizes clean Makefiles, executable commands, conformance test suites, and TDD-style specs as foundational for agent workflows" href="https://addyosmani.com/blog/good-spec/" link-text="Osmani: Good Specs for AI Agents →">direct consequence</tip> of how the search works. The <tip t="Zajac provides an explicit checklist: clear directory structure, CLAUDE.md files, clean Makefiles, testing safety nets" href="https://domizajac.medium.com/is-your-repo-ready-for-the-ai-agents-revolution-926e548da528" link-text="Zajac: Is Your Repo Ready? →">environment is the product</tip>, not the agent.

This applies to tools, too. When GeoEngine gave <tip t="Llama 3.1 8B with all 46 tools failed; with only 19 relevant tools, it succeeded. Irrelevant tool definitions consumed attention and degraded tool selection." href="https://datagrid.com/blog/optimize-ai-agent-context-windows-attention" link-text="GeoEngine: Tool Overload →">Llama 3.1 all 46 available tools</tip>, it failed. With only 19 relevant tools, it succeeded. Irrelevant tool definitions are noise in the field, just like irrelevant files. Some teams are moving toward <tip t="Short-lived, clean workspaces per agent prevent context accumulation across sessions" href="https://coder.com/blog/ephemeral-environments" link-text="Coder: Ephemeral Environments →">ephemeral environments</tip>: short-lived, clean workspaces spun up per agent, precisely to prevent accumulated noise from deforming subsequent searches.

**Feedback is the runtime reward signal.** Tests, linters, build outputs, and error messages are all part of the environment. They enter $s_t$ at each step and re-condition the field. This is the inference-time equivalent of the RL training reward: the signal the agent uses to navigate. During training, the model learned to chase verifier signals. At runtime, your test suite plays that role.

The quality of your feedback determines the quality of the search. Strong tests produce strong gradients: the agent can distinguish good trajectories from bad ones and course-correct. Weak tests produce weak signal: the agent optimizes for passing weak tests (and succeeds, which is worse than failing). Absent feedback produces blind search. As <tip t="Fireworks AI: 'most RL bugs are actually environment or integration bugs.' The feedback infrastructure matters as much as the model." href="https://fireworks.ai/blog/best-practices-for-multi-turn-RL" link-text="Fireworks: Best Practices for Multi-Turn RL →">Fireworks AI put it</tip>, "most RL bugs are actually environment or integration bugs." The Anthropic compiler project used GCC's torture test suite, 30 years of accumulated edge cases, as its verifier. That is not incidental to the project's success. It *is* the reason the search converged.

**Permissions are hard walls.** The system prompt and feedback bias the search. They make certain trajectories more or less likely, but they do not make them impossible. Permissions are different. They physically eliminate trajectories from the reachable space. If the agent has no write access to production, no trajectory through production exists, regardless of what the policy would prefer.

This matters because RL-trained agents find and exploit any accessible path to reward. This is not a bug. It is the trained behavior working as designed. The <tip t="OWASP Agentic AI Top 10 (Dec 2025) describes ASI01 (Agent Goal Hijack) as the condition where environment inputs override or redirect the system prompt objective, and ASI03 (Identity & Privilege Abuse) as agents exploiting inherited permissions" href="https://owasp.org/www-project-agentic-ai-threats-and-safeguards/" link-text="OWASP Agentic Top 10 →">OWASP Agentic Top 10</tip> catalogs the consequences: goal hijacking, privilege abuse, and tool misuse all stem from agents having access to trajectories that lead toward reward through unintended paths.

OpenClaw made this concrete. The platform stored API keys, gateway tokens, and encryption keys in <tip t="Infostealer malware (Vidar, RedLine, Lumma) began specifically targeting OpenClaw's .openclaw directory, harvesting openclaw.json and device.json files containing credentials and private keys" href="https://www.bleepingcomputer.com/news/security/infostealer-malware-found-stealing-openclaw-secrets-for-first-time/" link-text="BleepingComputer: Infostealer Steals OpenClaw Secrets →">plaintext JSON files</tip> accessible to any process in the agent's environment. Researchers found <tip t="Snyk scanned ~3,984 ClawHub skills and found 283 (~7.1%) contained flaws exposing API keys, passwords, and PII. 341 malicious skills were actively exfiltrating credentials including crypto wallet keys and SSH secrets." href="https://snyk.io/blog/openclaw-skills-credential-leaks-research/" link-text="Snyk: OpenClaw Skills Credential Leaks →">7% of ClawHub skills</tip> leaking credentials, and over <tip t="Security analysts reported 30,000+ compromised OpenClaw installations and 1,800+ exposed instances leaking Anthropic API keys, Telegram bot tokens, and Slack OAuth credentials" href="https://venturebeat.com/security/openclaw-agentic-ai-security-risk-ciso-guide/" link-text="VentureBeat: OpenClaw Agentic AI Security →">30,000 compromised installations</tip> leaking API keys and OAuth tokens. The secrets were not exfiltrated because someone instructed the agent to steal. They were exfiltrated because the shortest path through accessible permissions happened to run through those credentials, and <tip t="NVIDIA recommends network egress controls, blocking writes outside workspace, and credential brokers with short-lived tokens for agentic sandboxing" href="https://developer.nvidia.com/blog/practical-security-guidance-for-sandboxing-agentic-workflows-and-managing-execution-risk/" link-text="NVIDIA: Sandboxing Agentic Workflows →">nothing prevented access</tip>.

The practical response follows from the field theory. You want to eliminate trajectories, not just discourage them. <tip t="Auth0/Okta reported 92% reduction in credential theft with short-lived 300-second tokens vs 24-hour sessions" href="https://auth0.com/blog/access-control-in-the-era-of-ai-agents/" link-text="Auth0: Access Control for AI Agents →">Short-lived tokens</tip> (Auth0 reported 92% reduction in credential theft with 300-second tokens vs. 24-hour sessions). Workspace-scoped writes so the agent cannot touch anything outside its designated area. Network egress controls so credentials cannot leave the sandbox even if accessed. <tip t="Cerbos argues that traditional RBAC for humans is insufficient for agents; advocates fine-grained access control (FGAC) that blocks any tool call not explicitly scoped" href="https://www.cerbos.dev/blog/permission-management-for-ai-agents" link-text="Cerbos: Permissions for AI Agents →">Fine-grained access control</tip> that blocks any tool call not explicitly scoped. These are not security best practices bolted onto your agent workflow. In the search framework, they are hard walls that define the boundary of the reachable space. Design them first, not last.
