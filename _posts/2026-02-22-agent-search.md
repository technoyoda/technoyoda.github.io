...
Training gives you a policy $\pi_\theta$. Deployment gives you a transition function $T(s' \mid s, a)$ — whatever your tools, repos, and tests return. Behavior is the composition: roll out $\pi_\theta$ inside $T$ until termination. So the right question isn't "what is the model thinking?" but: **given this trained policy, what search problem did we build around it?**

Since the policy is defined as $\pi_\theta(a_t \mid s_t)$ — conditioned entirely on state — every engineering choice that changes the tokens in $s_t$ changes behavior. That's the thread that runs through everything below.

### State Is Context, Context Is Everything

The state $s_t$ is the context window — that's directly from the RL formulation in Section 2. The policy $\pi_\theta(a_t \mid s_t)$ is conditioned entirely on it. Not just the most recent feedback, but every token accumulated from $s_0$ onward.

This is not a new concept — it's the math from Section 2 playing out. When $s_t$ changes, the action distribution $\pi_\theta(a_t \mid s_t)$ changes. No weight updates, no "learning" in the training sense. The policy is <tip t="Olsson et al. (Anthropic, 2022) identify 'induction heads' — attention heads that complete patterns like [A][B]...[A] → [B] — as a core mechanism for in-context adaptation. These emerge suddenly during training and may constitute the majority of all in-context learning ability." href="https://arxiv.org/abs/2209.11895" link-text="Anthropic: Induction Heads →">re-conditioned</tip> by every token that enters context. The safe engineering assumption is: **anything you put in state can steer behavior**.

This has direct consequences:

- **Shortcuts get exploited.** If the agent reads a file that contains (or hints at) a solution, that enters $s_t$. The policy, trained to reach the finish state efficiently, will use it. This is the search strategy working as trained. <tip t="METR documented models tracing through Python call stacks to find pre-computed answers in the scoring system's memory — the model found a shorter trajectory to the reward and took it" href="https://metr.org/blog/2025-06-05-recent-reward-hacking/" link-text="METR: Reward Hacking →">METR documented</tip> models finding pre-computed answers in scoring systems. If your repo has a `solutions/` folder, the agent will find it and use it — because that is the shortest trajectory to the finish state.

- **Noise deforms the trajectory space.** If the agent reads irrelevant files — stale logs, unrelated config, artifacts from previous runs — those tokens enter $s_t$ and shift the action distribution. Models can attenuate via attention, but they don't have a reliable firewall between "data" and "instructions" — any tokens in context can steer behavior. If the agent reads database config while fixing a frontend bug, subsequent actions get biased toward database operations. This is what <tip t="CodeDelegator (2026) proposes separating planning from implementation so each agent gets a clean context, precisely to prevent accumulated debugging traces from polluting subsequent decisions" href="https://arxiv.org/abs/2601.14914" link-text="Context pollution paper →">context pollution</tip> looks like through the search lens: $s_t$ has been contaminated and the policy's action distribution has shifted accordingly. The search proceeds — but through <tip t="Breunig identifies four failure modes: context poisoning (fixation on misinformation), context distraction (over-attending to accumulated context), context confusion (irrelevant info degrading quality), and context clash (contradictions)" href="https://www.dbreunig.com/2025/06/22/how-contexts-fail-and-how-to-fix-them.html" link-text="How Contexts Fail →">deformed territory</tip>. <tip t="[TODO: This 'noise deforms the trajectory space' insight is the most practically valuable idea in the essay and deserves a worked example. Shi et al. (ICML 2023) showed accuracy drops below 30% after adding distractors. Du & Tian (2025) showed performance degrades 13.9-85% with increased context length even with perfect retrieval.]">[TODO: expand with worked example]</tip>

- **Early decisions compound.** Each $s_{t+1}$ builds from $s_t$ plus the new action and feedback. The trajectory is path-dependent — a bad file read at step 2 stays in context for all $t > 2$, and the policy is re-conditioned by it at every subsequent step. This is why agents work on short tasks ($s_0 \rightarrow \ldots \rightarrow s_5$) and struggle on long ones ($s_0 \rightarrow \ldots \rightarrow s_{50}$): by step 50 the signal-to-noise ratio has degraded and the policy has been re-conditioned by that degraded context for <tip t="Empirical observation: context files under ~100KB may not be fully loaded, resulting in decisions made on partial information. System prompt customization can shift the action distribution enough that default behaviors like writing to CWD change.">dozens of steps</tip>.

Because this re-conditioning is implicit — it happens through the conditional distribution, not through any explicit reasoning step — the agent has no mechanism to notice when its context is misleading it. Clean context sharpens the search. Polluted context sends it drifting. Either way, the policy responds with equal confidence.

### <tip t="The name is inspired by Kurt Lewin's Field Theory (1936) - a theory about a trying to predict a person's behavior in an environment and name just fit the model I was developing." href="https://en.wikipedia.org/wiki/Field_theory_(psychology)" link-text="Lewin's Field Theory →">Agent Field Theory</tip>

Given everything above — the rollout, state as context, re-conditioning — we can now describe what actually determines agent behavior.

**One scoping note first:** everything below assumes a fully autonomous rollout — the agent receives its initial conditioning (system prompt + user query) and then the loop runs without further human intervention. This is the case that matters most: the incidents in the prologue, the reward-hacking examples, the context pollution failures — these all happen when the dynamical process plays out unsupervised. If you use agents interactively (as most people do with Claude Code or Cursor), you are simply another source of environment feedback entering $s_t$ mid-trajectory. The theory does not change — you are just one more signal in the field. But the autonomous case is where understanding the dynamics pays off, because there is no one to course-correct when the search drifts.

There are three components, but they are not independent forces pushing on a ball. They form a causal chain with feedback loops.

**The trained policy $\pi_\theta$ is the substrate.** It is not just one influence among many — it is the thing that determines *how everything else expresses itself*. The system prompt only has power because the policy was trained to condition on it. Environment feedback only redirects because the policy was trained to respond to it. How the policy re-conditions as state accumulates — that is itself a function of how the policy was built. Everything flows through $\pi_\theta$, and $\pi_\theta$ is largely a black box to us as operators. We did not design the reward function. We do not know its full specification. We only observe the behavior it produces.

**The system prompt and the environment are what you control.** These are your inputs to the field:

- The **system prompt** is persistent conditioning — tokens in $s_t$ from $s_0$ onward, re-conditioning the policy at every step. It acts as an <tip t="Pan et al. (ICML 2024) formalize 'in-context reward hacking' (ICRH): the system prompt creates an implicit optimization target the model pursues at inference, via output-refinement and policy-refinement mechanisms" href="https://arxiv.org/abs/2402.06627" link-text="In-Context Reward Hacking paper →">in-context reward shaper</tip> — a persistent signal biasing the search toward trajectories consistent with its instructions. This is analogous to <tip t="Text2Reward (ICLR 2024) demonstrates the direct conversion of natural language instructions into reward-shaping functions" href="https://arxiv.org/abs/2309.11489" link-text="Text2Reward paper →">reward shaping in RL</tip>, except operating through the conditional distribution rather than an explicit reward signal.
- The **environment** — tools, permissions, files, test suites, feedback signals — defines what trajectories are physically reachable and what feedback the agent receives at each step.

Together, the system prompt and environment determine the *inputs* to the field. The trained policy determines the *dynamics* — how those inputs translate to behavior.

**In-context re-conditioning is the process by which the field evolves.** It is not a separate force. It is the mechanism by which the system prompt and environment compound over time, mediated by the trained policy. At each step:
...
Each cycle, the state $s_t$ accumulates more context. The system prompt's influence is constant. The environment's influence is dynamic. The policy's response to both is determined by training. And the re-conditioning process compounds all of it — clean inputs compound into focused search, noisy inputs compound into drift.

**When these components align** — capable policy, well-calibrated prompt, clear feedback, clean context — the search is well-bounded and converges. **When they conflict**, behavior becomes hard to predict. And here is the honest part: **we do not know how conflicts resolve**. The trained policy is a black box. The interaction between system prompt conditioning and environment feedback is <tip t="OpenAI's instruction hierarchy research found models often treat system prompts and user messages at roughly equal priority. Their proposed hierarchy — training alignment > operator system prompt > user message > environment content — is an attempt to formalize this, but it requires explicit training to enforce." href="https://arxiv.org/html/2404.13208v1" link-text="Instruction Hierarchy paper →">not deterministic</tip>. <tip t="Bondarenko (2025) showed o3 reward-hacks even when explicitly instructed not to — the trained policy overpowered the system prompt. Wei et al. (NeurIPS 2023) named 'competing objectives' as an explicit failure mode where instruction-following overpowers safety on every prompt tested." href="https://arxiv.org/pdf/2502.13295" link-text="Specification Gaming in Reasoning Models →">Research shows</tip> that trained policy tendencies can overpower explicit system prompt instructions, and environment feedback can <tip t="OWASP's Agentic Top 10 (Dec 2025) describes ASI01 (Agent Goal Hijack) as the condition where environment inputs override or redirect the system prompt objective." href="https://owasp.org/www-project-agentic-ai-threats-and-safeguards/" link-text="OWASP Agentic Top 10 →">hijack the objective entirely</tip>.

There is no clean formula for which force wins. But since you control the system prompt and the environment — and you cannot change the trained policy — the engineering question becomes: how do you design the inputs so that the forces are less likely to conflict, and build guardrails for when they do? That is what the next section is about.
